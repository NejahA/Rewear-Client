...
const Create = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        setItem(prev => ({ ...prev, user: res.data._id }));
      })
      .catch((err) => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_VERCEL_URI}/api/items`, formData, {
      ...
      setErrors(err.response?.data?.errors || { message: "Failed to create item" });
      Swal.fire({
        icon: 'error',
        title: 'Erreur',
        text: err.response?.data?.errors?.message || 'Failed to create item',
      });
    }
  };
